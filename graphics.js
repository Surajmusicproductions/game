...
export class InstancedGrass {
    ...
    update(delta, time) {
        // Simple wind effect
        this.mesh.rotation.y = Math.sin(time * 0.5) * 0.02;
    }
    dispose(){ this.mesh.geometry.dispose(); this.mesh.material.dispose(); }
}

// 7) AdaptiveQuality — simple performance monitor
export class AdaptiveQuality {
    ...
    update() {
        ...
        if (now - this.lastCheck > 2000) { // Check every 2 seconds
            this.avgFPS = (this.frameCount * 1000) / (now - this.lastCheck);
            this.frameCount = 0;
            this.lastCheck = now;
            
            if (this.avgFPS < 30 && this.highQuality) {
                this.highQuality = false;
                this.applyLowQuality();
            } else if (this.avgFPS > 50 && !this.highQuality) {
                this.highQuality = true;
