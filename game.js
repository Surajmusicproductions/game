...
class ForestRunnerGame {
    ...
    setupEngine() {
        ...
        this.post = { bloomPass }; // For adaptive quality control
    ...
    setupControls() {
        document.addEventListener('keydown', (e) => {
            ...
            if (e.code === 'Space' && !this.isJumping && !this.isCrouching) { this.isJumping = true; this.jumpVelocity = this.JUMP_FORCE; }
            if (e.key === 'Shift' && !this.isJumping) { this.isCrouching = true; this.camera.position.y = this.CROUCH_HEIGHT; }
        });
        document.addEventListener('keyup', (e) => {
            if (e.key === 'a' || e.key === 'ArrowLeft') this.controls.moveLeft = false;
            if (e.key === 'd' || e.key === 'ArrowRight') this.controls.moveRight = false;
            if (e.key === 'Shift') { this.isCrouching = false; if (!this.isJumping) this.camera.position.y = this.NORMAL_HEIGHT; }
        });
    ...
    resetGame() {
        ...
        this.worldChunks.forEach(chunk => {
            chunk.userData.animated.forEach(obj => obj.dispose());
    ...
    createJumpingObstacle(x, z) {
        const log = new THREE.Mesh( new THREE.CylinderGeometry(0.5, 0.5, 4, 6).rotateZ(Math.PI / 2), this.groundMaterial);
        log.castShadow = true;
    ...
    createPerk(type, x, z) {
        let geo, mat;
        if (type === 'collision') { geo = new THREE.OctahedronGeometry(1); mat = new THREE.MeshStandardMaterial({ color: 0x00ff00, emissive: 0x00ff00, emissiveIntensity: 0.5 }); }
        else { geo = new THREE.TorusGeometry(0.8, 0.3, 8, 12); mat = new THREE.MeshStandardMaterial({ color: 0x4444ff, emissive: 0x4444ff, emissiveIntensity: 0.5 }); }
    ...
    createWorldChunk(chunkX, chunkZ) {
        ...
        for(let i = 0; i < obstacleCount; i++) {
            ...
            if (this.difficultyLevel < 2 || pattern > 0.8) {
                ...
                chunkGroup.userData.obstacles.push(...tree.group.children.filter(c => c.userData.isObstacle));
        ...
        for (let i = 0; i < detailCount; i++) {
            ...
            chunkGroup.userData.animated.push(foliage); // for disposal
        }
    ...
    update(delta, time) {
        ...
        if (this.score > this.highScore) { this.highScore = this.score; localStorage.setItem('forestRunnerHighScore', this.highScore); }
        ...
        if (this.isInvincible && time > this.invincibilityEndTime) { this.isInvincible = false; }
        let speedMultiplier = 1.0;
        if (this.slowTimeActive && time < this.slowTimeEndTime) { speedMultiplier = 0.5; }
        else if (this.slowTimeActive) { this.slowTimeActive = false; this.updatePerkUI(); }
        ...
        this.worldChunks.forEach((chunk, chunkId) => {
            ...
            if (Math.abs(cx - camChunkX) > this.RENDER_DISTANCE + 1 || Math.abs(cz - camChunkZ) > this.RENDER_DISTANCE + 1) {
                ...
                // Animate objects in visible chunks
                chunk.userData.animated.forEach(obj => obj.update(delta, time));
            }
        });
